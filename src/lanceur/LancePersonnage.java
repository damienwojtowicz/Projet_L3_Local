...
public class LancePersonnage {
	...
	public static void lancerPerso(String[] args) {
		...
		for(int i = 0 ; i<nom.length ; i++){
			...
			try {
				logger = new LoggerProjet(true, "personnage_" + nom[i] + groupe);
			} catch (IOException e) {
				e.printStackTrace();
			...
			try {
				...
				new StrategiePersonnage(ipArene, port, ipConsole, nbTours, position, logger, new Personnage(nom[i],groupe,caracts));
